...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($result && $result->num_rows === 1) {
        ...
        if ($passwordValid) {
            ...
            // Audit: super admin login success
            logActivity($conn, $admin['user_id'] ?? null, 'super_admin', 'login', null, null, ['status' => 'success']);
...
<head>
    ...
    <title>E-Notice - Admin Login</title>
...
<body class="bg-background font-body-md text-on-background academic-mesh min-h-screen flex items-center justify-center p-6">
    <main class="w-full max-w-[1100px] grid grid-cols-1 md:grid-cols-12 bg-white rounded-xl shadow-xl shadow-slate-200/50 overflow-hidden border border-outline-variant">
        ...
        <section class="md:col-span-7 p-12 bg-white flex flex-col justify-center">
            <div class="mb-10">
                <h2 class="font-h2 text-3xl text-primary mb-2">Super Admin Login</h2>
                <p class="font-body-md text-base text-on-surface-variant">Enter your administrator credentials to access the control panel.</p>
            </div>
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div class="relative">
                        ...
                        <div class="relative group">
                            <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-outline group-focus-within:text-secondary">admin_panel_settings</span>
                            <input class="w-full pl-12 pr-4 py-3.5 bg-surface-container-low border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-secondary/20 focus:border-secondary outline-none transition-all placeholder:text-outline-variant"
                                id="email" name="email" type="email" placeholder="e.g. admin@university.edu"
