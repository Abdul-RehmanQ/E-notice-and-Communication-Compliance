...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_post'])) {
    ...
    if ($stmt->execute() && $stmt->affected_rows > 0) {
        communityRedirectWithFlash('success', 'Post deleted successfully!');
    } else {
        communityRedirectWithFlash('error', 'Failed to delete post or post not found.');
    }
    $stmt->close();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['create_post'])) {
    ...
    if (!$includeText && !$includeImage) {
        ...
        if (empty($error)) {
            ...
            if ($stmt->execute()) {
                communityRedirectWithFlash('success', "Post created successfully! It will expire in $expiresIn days.");
            } else {
    ...
    if ($error !== '') {
        communityRedirectWithFlash('error', $error);
    }
}
...
// Fetch approved posts visible to this teacher (university-wide + own department)
$posts = [];
...
// Fetch teacher's own posts for delete dropdown
$userPosts = [];
...
<body class="bg-light">
    ...
    <script>
        ...
        document.getElementById('includeText').addEventListener('change', function() {
            document.getElementById('textSection').classList.toggle('d-none', !this.checked);
        });
        ...
        // Reset add post modal when closed
